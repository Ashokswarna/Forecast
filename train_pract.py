# -*- coding: utf-8 -*-
"""
Created on Tue Jan  8 15:29:56 2019
...
def train(filename):
   ...
   #df = pd.read_excel(file_path)
    ...
    sku_group = df.groupby('FUCode', as_index=False)
    ...
    for sku in sku_list:
        ...
        for tp in train_period:
            ...
            train_sales = df_train_period['Actuals'].reset_index(drop=True)
            validation_sales = df_validation_period['Actuals'].reset_index(drop=True)
            mse_sales = df_mse_period['Actuals'].reset_index(drop=True)
...
import pickle
with open('C:\\Users\\ashok.swarna\\Downloads\\model.pickle', 'rb') as fp:
...
#keep the below line in for loop and break when u got the date field
#2nd date field will get ignored 

for col in df.columns:
    print ('col = ', col)
    a = isinstance(df[col], datetime.date)
    if a == True:
        print ('Date field :- ', col  )        
    else:
        print ('field name :- ', col, 'filed type :- ', type(col) )
   
a = list()    
df.dtypes  
a = datetime.datetime.now()

b = isinstance(a, datetime.date)
